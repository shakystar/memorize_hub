...
있고, 이 컴포넌트 전체는 HP1/HP2 이후로 연기된다.
...
이 문서는 미래 read/write surface를 **금지하지 않는다** - 단지 relay·gateway와 분리하고
지금은 연기할 뿐이다. 그 착수 트리거(첫 claude.ai 소비자 / 로컬에 안 담기는 코퍼스 /
브라우저 workspace UI 수요)는 미결이다([[H900]]). 무엇을 허용/거부하는가의 인가는
