...
> **구현 후 갱신(2026-07-05):** 최초 time-first 모델은 유지되나, 렌더 구조는 라이브
> 리뷰를 거쳐 Notion Timeline식(카드 막대·무한 팬·Today)으로 진화했다. 상세는 **§8**.
...
- **시간축 팬**: ~~v1은 fit-to-span만.~~ **SUPERSEDED(§8, 2026-07-05)** — 사용자 요청으로
  무한 양방향 스크롤-팬을 구현. **줌**(px/일 가변)은 여전히 유보(고정 px/일).
...
## 8. 구현 후 설계 변경 (2026-07-05, 라이브 리뷰)

와이어 계약(§2)·replica 유도(§3)·헬퍼(§4.2)·앵커 규칙(§4.3)은 **그대로**다. 바뀐 건
**web 렌더 구조**뿐 — mock 대상 라이브 리뷰에서 사용자가 Notion Timeline식을 요구.

**전달된 구조 (`TasksTimeline.tsx`):**

1. **카드/막대 분리 렌더** — 좌측 라벨 열(초안 §4.1의 표+간트) 폐기. task당: 진짜-시간-스케일
   세그먼트 막대(대기=윤곽 트랙 / 작업=상태색) + **제목은 막대 오른쪽으로 흘림**(막대 안에
   넣으면 min-width 때문에 막대가 실제 기간과 분리돼 대기\|작업 경계가 시간축·화살표와
   어긋남 → 제목을 막대 밖으로 빼서 해결. 대안 B "제목 막대 위"는 유보).
2. **무한 양방향 스크롤-팬** — 고정 `PX_PER_DAY`(140), 차트가 팬보다 넓어 가로 스크롤.
   휠/트랙패드 세로 델타를 가로로 매핑(네이티브 non-passive 리스너로 `preventDefault`),
   **스크롤바 UI 숨김**. 스크롤이 엣지(±`EDGE_PX`)에 닿으면 시간창을 `CHUNK`(30일)씩 확장 —
   좌측 확장 시 `scrollLeft`를 델타만큼 보정해 뷰 점프 방지.
3. **상단 날짜축 + Today** — 일 눈금·월 라벨, 오늘 = 빨간 원+세로선. **Today 버튼**으로
   오늘로 부드럽게 스크롤(초기 뷰도 now 정렬).
4. **화살표 z-order** — dependsOn 화살표를 막대 **위** 레이어(맨 앞 SVG)로 그려 안 묻히게.
   Notion식 주황 곡선, 도착점은 여전히 `startedAt ?? createdAt`(§4.3).
5. **가독성** — 대기 트랙 `fill-secondary`(배경 동색)가 안 보여 `accent`+`border` 윤곽으로,
   작업 세그먼트는 status별 색(done=회색이라 대기와 구분). mock 4건 `startedAt`을 벌려
   대기→작업 분절 가시화.

**여전히 유보(후속):** 그룹핑(현재 flat; 소스 프로젝트별은 후속), 시간축 **줌**(px/일 가변),
다중 작업 세그먼트, 라이브 데이터 검증(배포 후). §1.2의 "팬" 항목은 여기서 superseded.

렌더 구조 변경은 **순수 헬퍼·와이어·replica 로직 무변경**이라 단위 테스트(10 web + replica)는
그대로 green. 스크롤/무한-팬은 브라우저 상호작용이라 유닛 미검증 — 배포 후 시각 확인 대상.
